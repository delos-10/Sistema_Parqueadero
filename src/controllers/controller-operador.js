...
function abrirTurno() {
  ...
  if (turnoActivo) {
    ...
    msg(
      `⚠️ Ya hay un turno activo (<b>${operadorTurnoActual.nombre}</b>). Lleva <b>${dur}</b> abierto.`,
      "warn",
      "panelOpResult",
    );
    return;
  ...
  msg(
    `✅ Turno abierto a las <b>${horaInicioTurno.toLocaleTimeString("es-CO")}</b>. Operador: <b>${sesionActual.nombre}</b>`,
    "ok",
    "panelOpResult",
  );
}
...
function reporteRapido() {
  ...
  const dur = turnoActivo
    ? formatDur(new Date() - horaInicioTurno)
    : "Sin turno activo";
...
function buscarVehiculo() {
  ...
  if (vehiculos[placaUp]) {
    ...
    if (reservaEncontrada) {
      msg(
        `ℹ️ Placa <b>${placaUp}</b> tiene reserva <b>${reservaEncontrada.id}</b> en estado <b>${reservaEncontrada.estado}</b>. No está físicamente dentro.`,
        "info",
        "panelOpResult",
      );
    } else {
      msg(
        `⚠️ La placa <b>${placaUp}</b> no está registrada como activa ni tiene reservas.`,
        "warn",
        "panelOpResult",
      );
    }
