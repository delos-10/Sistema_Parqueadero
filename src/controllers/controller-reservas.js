...
function crearReservaUsuario() {
  ...
  if (horaEntrada >= horaSalida) {
    msg('⚠️ La hora de salida debe ser posterior a la de entrada.', 'warn', 'resultadoReserva');
    return;
  }
...
function limpiarFormularioReserva() {
  ['resPlaca','resMarca','resModelo','resColor','resHoraEntrada','resHoraSalida'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  const hint = document.getElementById('placaHint2');
  if (hint) { hint.textContent = 'Solo letras y números (ej: ABC123)'; hint.className = 'placa-hint'; }
}
...
function renderMisReservas() {
  ...
  const propias = reservasDeUsuario(sesionActual.usuario).filter(
    r => ['pendiente', 'confirmada', 'activa'].includes(r.estado)
  );

  if (propias.length === 0) {
    cont.innerHTML = '<div class="hist-empty">📭 No tienes reservas activas o pendientes</div>';
    return;
  }
  cont.innerHTML = '';
...
function renderHistorialUsuario() {
  ...
  if (hist.length === 0) {
    cont.innerHTML = '<div class="hist-empty">📭 Aún no tienes reservas finalizadas o canceladas</div>';
    return;
  }
  cont.innerHTML = '';
...
function renderTarjetaReservaUsuario(r, contenedor) {
  ...
  const costoTxt = r.costoFinal != null
...
function abrirEdicionReserva(id) {
  if (!tienePermiso(['usuario', 'administrador'])) return;
  const r = buscarReservaPorId(id);
  if (!r || r.estado !== 'pendiente' || r.usuario !== sesionActual.usuario) return;
...
function guardarEdicionReserva() {
  ...
  const placa       = document.getElementById('editResPlaca').value.trim().toUpperCase().replace(/\s/g, '');
  ...
  if (!placa || !marca || !modelo || !color || !fecha || !horaEntrada || !horaSalida) {
    msg('⚠️ Completa todos los campos.', 'warn', 'editResMsg');
    return;
  }
  if (horaEntrada >= horaSalida) {
    msg('⚠️ La hora de salida debe ser posterior a la de entrada.', 'warn', 'editResMsg');
    return;
  }

  editarReserva(id, { placa, marca, modelo, color, fecha, horaEntrada, horaSalida });
  PZ.guardarReservas();
  cerrarEdicionReserva();
  renderMisReservas();
  renderReservasOperador();
  msg(`✅ Reserva <b>${id}</b> actualizada correctamente.`, 'ok', 'resultadoReserva');
}
...
function consultarMiVehiculo() {
  ...
  if (vehiculos[placa]) {
    ...
    `, 'ok', 'resultadoMiVehiculo');
  } else {
    const reservaPendiente = reservas.find(
      r => r.placa === placa && ['pendiente', 'confirmada'].includes(r.estado) && r.usuario === sesionActual.usuario
    );
    if (reservaPendiente) {
      msg(`⏳ <b>${placa}</b> tiene una reserva <b>${reservaPendiente.estado}</b> para el ${reservaPendiente.fechaReserva}. Aún no ha ingresado.`, 'info', 'resultadoMiVehiculo');
    } else {
      msg(`⚪ <b>${placa}</b> no está registrada dentro del parqueadero en este momento.`, 'warn', 'resultadoMiVehiculo');
    }
  }
}
