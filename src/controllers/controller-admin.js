...
function configurarTarifa() {
  ...
  document.getElementById("tarifaActualShow").textContent =
    `${formatCOP(TARIFA_MIN)}/min`;
...
function guardarTarifa() {
  ...
  PZ.guardarConfig();
  msg(
    `✅ Tarifa actualizada a <b>${formatCOP(TARIFA_MIN)}/min</b>. Cobro mínimo: <b>${formatCOP(COBRO_MINIMO)}</b>`,
    "ok",
    "panelAdmResult",
  );
  cerrarTarifa();
...
function configurarCapacidad() {
  if (!tienePermiso(["administrador"])) return;
  const nueva = parseInt(
    prompt(
      `Capacidad actual: ${CAPACIDAD} puestos.\nIngresa la nueva capacidad (1-500):`,
    ),
  );
  if (isNaN(nueva) || nueva < 1 || nueva > 500) {
    msg(
      "⚠️ Capacidad inválida. Debe ser entre 1 y 500.",
      "warn",
      "panelAdmResult",
    );
    return;
  ...
  actualizarStats();
  msg(
    `✅ Capacidad actualizada a <b>${CAPACIDAD} puestos</b>.`,
    "ok",
    "panelAdmResult",
  );
}
...
function resetearSistema() {
  ...
  if (
    !confirm(
      "⚠️ ¿Seguro que deseas resetear el sistema?\nSe borrará TODO el registro, incluyendo reservas, turnos e historial.",
    )
  )
    return;
  ...
  msg(
    `🔄 Sistema reseteado correctamente a las <b>${new Date().toLocaleTimeString("es-CO")}</b>.`,
    "warn",
    "panelAdmResult",
  );
}
...
function abrirDashboard() {
  ...
  if (turnoActivo && operadorTurnoActual)
    operadoresActivos.add(operadorTurnoActual.usuario);
...
function calcularYRenderNomina() {
  const tarifaInput = parseInt(
    document.getElementById("nominaTarifaInput").value,
  );
  ...
  if (filas.length === 0) {
    cont.innerHTML =
      '<div class="hist-empty">Aún no hay turnos registrados para calcular la nómina.</div>';
  ...
  filas.forEach((f) => {
    ...
    const horaInicio =
      turnoActivo &&
      operadorTurnoActual &&
      operadorTurnoActual.usuario === f.operador
  ...
  document.getElementById("nominaTotalGeneral").textContent =
...
document.getElementById("overlayDashboard").addEventListener("click", (e) => {
  if (e.target === document.getElementById("overlayDashboard"))
    cerrarDashboard();
});
...
document.getElementById("overlayEditReserva").addEventListener("click", (e) => {
  if (e.target === document.getElementById("overlayEditReserva"))
